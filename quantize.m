function quantizeima = quantize(image)
...
for row = 1:size(index, 1)
    ...
    quantizeima(index(row, 1), index(row, 2), index(row, 3)) = ... 
        quantizeima(index(row, 1), index(row, 2), index(row, 3)) + 1;
end


quantizeima = quantizeima(:)';
quantizeima = quantizeima/sum(quantizeima);
